...
# Buttons to choose for the Graphical Summaries
                     ...
                     conditionalPanel(
                       ...
                       checkboxGroupInput(inputId = "rgn", label = "Which region would you like to see? Choose All OR a selection of region", choices = c("northeast","northwest","southeast","southwest"), selected = NULL)),
...
tabPanel("Modelling",
         p("This is the modelling phase of the app. The purpose of this phase is to produce supervised learning models with the data to show the interation between medical bill charges and the explanatory variables of contractor gender, age, number of children, region of residence, body mass index,and smoker status. The two models used are multiple linear regression and random forest. The Model Info subtab provides a description of each model - its purposes for analysis, advantages, and disadvantages when describing the data. The Model Fitting tab goes through the process of creating multple regression or regression tree models to the data. Finally, the prediction tab will allow you to use the explanatory variables to predict the outcome of the charges."),
...
#Select Tuning Parameters and Cross Validation settings for Random Forest    
    ...
    actionButton("fit", label = "Almost there ! Click here to fit the models")
        ...
        tabsetPanel(
          ...
          tabPanel("Random Forest Model Output", h1("Random Forest Model Output"),column(width = 3, verbatimTextOutput("rfsm"), verbatimTextOutput("rfimp")),column(width = 5, offset = 4,verbatimTextOutput("rfitstat"),plotOutput("rfplot")))
