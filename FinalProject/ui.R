...
# Introduction  
 ...
 tabPanel("About", h1("Welcome!"),
          ...
          h2("The purpose of this project is to produce a shiny app that could analyze and predict medical insurance costs given a series of health, familial, and regional variables. For many poeple in the United States, purchasing medical insurance is an expensive and frustrating task. However, having access to tools, such as an app that considers lifestyle, and regional factors that have an impact on costs but often go unnoticed, could be useful for customers in the purchasing process. The dataset chosen uses variables such as age, sex, body mass index, number of children that the customer has, whether or not they are a smoker, and which region in the United States they live in, to provide insight on medical costs billed by health insurance providers. The data was created as a tool to aid in practice exercises geared towards machine learning and regression techniques. More information and in-depth  descriptions of the variables can be found on the",a(href="https://www.kaggle.com/datasets/mirichoi0218/insurance","Kaggle webiste"),". Below are descriptions of the remaining tabs in the app."), 
          br(),
          h2("Data Exploration : The Data Exploration tab provides some prelminary exploratory data analysis datasets and visualizations that would allow the user to have an in depth introduction to the factors that have an impact on the response variable -charges (medical costs billed by health insurance companies) "), 
             br(),
          h2("Modelling : The Modelling tab allows the user to specify variables and parameter requirements that will aid in modelling and predicting the charges. Cross validation techniques  are used in some of the modelling. Thus, the user will have the option to choose the ratios for which they would like to split the data into train/test sets. The models used for this data will be multiple linear regression and random forest and the numercal variables are standardized since the units in the data set are different. Thus, for the random forest model, the user will have the option to choose the tuning  and cross validaion parmeters that will be used in the model. Finally, the Prediction tab will allow the user to view predicted values for the charges using the models."),
          br(),
...
#Model Info tab that provides introductions to the models
         ...
         tabPanel(" Modeling Info", fluidPage(
           ...
           helpText("Mutiple Linear Regrssion: Multiple Linear Regression is a form of supervised learning aimed to explain and predict the variation in a continuous respnse variable given a set of explanatory variables that can be categorical or numerical where the relationship between the response and the estimated parameters of the explanatory varaibles is linear. An example model can be seen below. $$Y = \\beta_{0} + \\beta_{1}x_{1} + \\beta_{2}x_{2} + \\beta{3}x^{2} + \\beta_{4}x_{1}x_{2} + \\epsilon$$"),
           helpText(' where the \\(\\beta_{0}\\) is the intercept of the regression that often consists of the mean value of the response, the rest of the \\(\\beta_{i}\\) terms are the estimated coefficients of the model, the x_{i} terms are the expanatory variables and the \\(\\epsilon\\) term is the sum of the  vertical distances between the estimated regression line and the data points that surround it ( also known as the error term). The model serves as an extension of simple linear regression since it can include polynomial and interaction terms in addition to the original explanatory variables. When fitting a multiple linear regression model, a main objective is for the sum of the squared deviations from the line and the data points to be as small as possible. This ensures that the estimates for the regression line explain as much of the variation as possible. Multiple linear regression is often used because it allows analysts to predict values for the response variable and to obeserve the contribution of each explanatory variable. However, some analysts find that multiple linear regression comes with strong assumptions that fail to translate to a variaty of data types. Some of these assumptions include the restriction that the error terms must follow a normal distribution with a constant variance, and that the main test types for determining the significance of the model and overall variables follow an F-test or a T-test. When these requirements are not met, analysts may turn to generalized linear models  where some of these restrictions are not required.'),
           helpText("Random Forest Model: The random forest model is another supervised machine learning technique, but its objective is to take a random sapmle of  decision trees(created using boostrapping sample methods), predict on the randomly sampled trees, and average over the predictions. Random forests are often used because the averaging helps to prevent overfitting of the data, they can reduce the variation in the estimation because the model seeks to construct independent trees, and the  predictions are often better overall than that of other tree based methods such as single tree models.To determine the appropriate random forest model, it is often the aim to reduce the average distance between the predicted and actual values - also known as the root mean square error. the calculation of the erorr is below: $$RMSE = \\sqrt{\\frac{1}N \\sum_{i=1}^n {\\hat{y_{i}} - y_{I}^2} }$$"),
