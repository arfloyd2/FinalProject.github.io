 ...
 # Output Summary
   ...
   # Fitting the Model
    if(is.null(input$MLRvars)){
       ...
       mlrdta <- storage$trainTransformed[, c("charges", isolate({input$MLRvars}))]
    ...
    # Split Data into Train and Test Set
       ...
       if(is.null(input$MLRvars)){
         ...
         data.frame(fitdta,compdta)
           ...
           trainTransformed1 <- storage$trainTransformed[, c("charges",isolate({input$MLRvars}))]
           testTransformed1 <- storage$testTransformed[, c("charges",isolate({input$MLRvars}))]
...
output$rfsm <- renderPrint(
  ...
  if(is.null(input$RFvars)){
    ...
    req(isolate({input$mtry}))
    req(isolate({input$fld}))

    rffit <- train(charges~., data = storage$trainTransformed, method="rf",
                   trControl = trainControl(method = "cv", number = as.numeric(isolate({input$fld})), search = "random"),
                   tuneGrid = data.frame(mtry = eval(parse(text = isolate({input$mtry})))))

    print(rffit$finalModel)
  }

  else{
    rfrdta <- storage$trainTransformed[, c("charges", isolate({input$RFvars}))]
...
output$rfplot <- renderPlot(
  ...
  if(is.null(input$RFvars)){
   ...
   #Multiple Linear Regression Prediction
    ...
    output$mlr1 <- renderPrint(

      summary(lm(charges ~ .,data = storage$testTransformed))


    )

      output$mlrpred <- renderText(

         paste0("The predicted amount of medical costs billed, using MLR, is $",


                round(predict(lm(charges~ .,data = storage$testTransformed)


                        , newdata = data.frame(age=(isolate({input$Age})), sex=(isolate({input$sx2})), bmi=(isolate({input$BMI})),children=(isolate({input$child})), smoker=(isolate({input$smk})), region = (isolate({input$rgn2})))),2)))
...
output$rf1 <- renderPrint(
  ...
  print(train(charges~., data = storage$testTransformed, method="rf",
                 trControl = trainControl(method = "cv", number = as.numeric(isolate({input$fld})), search = "random"),
                 tuneGrid = data.frame(mtry = eval(parse(text = isolate({input$mtry})))))))


output$rfpredt <- renderText(


  paste0("The predicted amount of medical costs billed, using Random Forest Modelling,  is $",
         round(predict(train(charges~., data = storage$testTransformed, method="rf",
                             trControl = trainControl(method = "cv", number = as.numeric(isolate({input$fld})), search = "random"),
                             tuneGrid = data.frame(mtry = eval(parse(text = isolate({input$mtry}))))),
                             newdata = data.frame(age=c(isolate({input$Age1})), sex=c(isolate({input$sx3})),
                            bmi=c(isolate({input$BMI1})),children=c(isolate({input$child1})),smoker=c(isolate({input$smk2})),
                            region = c(isolate({input$rgn3}))))
                                                                                                   ,2)))
